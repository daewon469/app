...
async function fetchSlidePostIdsFromApi() {
  ...
  for (let page = 0; page < 20; page++) {
    ...
    items.forEach((p) => {
      if (Number(p.card_type) === CARD_TYPE_S) ids.push(Number(p.id));
    });
...
export default function SlidePostsAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const configIds = Array.from(
        new Set<number>(
          ((res.config as any)?.slide_posts?.post_ids ?? [])
            .map((v: unknown) => Number(v))
            .filter((n: number): n is number => Number.isFinite(n) && n > 0),
        ),
      );
      const ids = configIds.length > 0 ? configIds : await fetchSlidePostIdsFromApi();
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 30 }} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <Text style={{ fontWeight: "900", marginBottom: 10 }}>
            현재 슬라이드 현장: {slidePosts.length}개
          </Text>
          ...
          {(slidePosts || []).map((p, index) => {
            ...
            return (
              ...
              >
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                  <Ionicons name="albums-outline" size={18} color="#2F6BFF" />
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontWeight: "900" }} numberOfLines={2}>
                      {p.title}
                    </Text>
