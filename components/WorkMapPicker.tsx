...
type Props = {
  ...
  business?: { address?: string; lat?: number; lng?: number };
};
...
export default function WorkMapPicker({
  ...
  const { work_lat, work_lng, work_address, work_zoom } = useSelector((state: RootState) => state.location);
  ...
  const html = useMemo(() => {
    const initLat = initial?.lat ?? work_lat ?? 37.5665;
    const initLng = initial?.lng ?? work_lng ?? 126.9780;
    const initZoom = initial?.zoom ?? work_zoom ?? 16;
    const initAddr = initial?.address ?? work_address ?? "";
    ...
    kakao.maps.load(function(){
      ...
      function runSearch(){
        ...
        ps.keywordSearch(query, function(data, status){
          ...
          // 검색 결과도 좌표 기준으로 역지오코딩을 한 번 더 수행(도로명 우선)
  ...
  }, [clientId, initial, work_address, work_lat, work_lng, work_zoom]);
            ...
            <Pressable
              onPress={() => {
                if (!business?.lat || !business?.lng || !business?.address) {
                  Alert.alert("사업지 주소 없음", "사업지 주소를 먼저 입력해주세요.");
                  return;
                }
