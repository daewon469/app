...
export default function IdPostCard7({ post }: Props) {
  const colors = {
    // idpostcard6.tsx와 동일 톤 유지
    background: "#fff",
    card: "#fff",
    text: "#000",
    border: "#000",
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
          ...
          <Card.Content>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <Text style={{ fontSize: 13, color: colors.subText }}>
                {post.author?.username ? post.author.username : "작성자"}
              </Text>
              <Text style={{ fontSize: 13, color: colors.subText }}>{createdAtText}</Text>
            </View>
          </Card.Content>
        </Card>
        ...
        >
          <Card.Content>
            <Text style={{ fontSize: 15, lineHeight: 22, color: colors.text }}>{post.content}</Text>
          </Card.Content>
        </Card>

        {!!post.agency_call?.trim?.() && (
          <Card
            mode="elevated"
            style={{
              borderRadius: 10,
          ...
          >
            <Card.Content>
              <Text style={{ fontSize: 14, color: colors.subText, marginBottom: 6 }}>연락처</Text>
              <Text style={{ fontSize: 16, fontWeight: "700", color: colors.text }}>{post.agency_call}</Text>
            </Card.Content>
          </Card>
        )}
      </ScrollView>
